...
if(isset($_GET['id']) && !empty($_GET['id'])){
	...
	$category_title = $fetch_result['title'];
	$category_id = $fetch_result['id'];
}
		...
		<div class="container-fluid">
			<div class="row">
					...
					<div class="dropright btn-block">
						...
						<?php
							while ($fetch_categories = mysqli_fetch_assoc($categories_result)){
								$fetched_category_title = $fetch_categories['title'];
								$ads_category_query = "SELECT * FROM ads WHERE category = '$fetched_category_title'";
					...
					<div class="collapse" id="collapsePrimjer">
						<div class="card card-body">
							<form id="sort_form" action="sort_ads_category.php" method="POST">
								...
								<div class="form-group">
									<label for="price_up_to">Price up to:</label>
									<input type="number" class="form-control" name="price_up_to" id="price_up_to">
								</div>
								<hr>
				...
				<div class="col-sm-10">
					...
					<hr/>
					<?php
						$cat_query = "SELECT * FROM ads WHERE category = '$category_title' ORDER BY id DESC";
						$cat_result = mysqli_query($link, $cat_query);
						$cat_counter = mysqli_num_rows($cat_result);
						if($cat_counter == 0){
							echo "<div style='margin-top: 10px; color:red; text-align: center'><strong>There are no ads to be displayed!</strong></div>";
						}
						else{
							while ($cat_fetch_result = mysqli_fetch_assoc($cat_result)){
						?>
								<a href="view_ad.php?id=<?php echo $cat_fetch_result['id']; ?>">
									<div class="display_ads">
										<div class="ad_picture">
											<img src="<?php if(isset($cat_fetch_result['ad_picture']) && !empty($cat_fetch_result['ad_picture'])){ echo $cat_fetch_result['ad_picture']; }else{ echo "../default_pictures/ad_picture.png";} ?>" alt="Ad picture" class="ad_picture_style"> 
										</div>
										<div class="ad_title">
											<?php
											$title = $cat_fetch_result['title'];
										...
										<div class="ad_price">
											<?php echo $cat_fetch_result['price']; ?> €
										</div>
										<div class="ad_publication_time">
											<?php echo $cat_fetch_result['publication_time']; ?>
										</div>
									</div>
								</a>
						<?php } } ?>
